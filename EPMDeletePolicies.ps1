<#
.SYNOPSIS
    Delete policies Inactive, created by JIT application

.DESCRIPTION
    By default a policies created by JIT application wil be deleted in 3 months, this script check the JIT created policy and remove if 1 month old (or any other value)
...
$policyFilter = @{
    "filter" = "PolicyName CONTAINS ""Temporary elevation"" AND IsActive EQ false"
    #"filter" = "IsActive EQ false"
}  | ConvertTo-Json

$policiesList = Invoke-EPMRestMethod -URI "$($login.managerURL)/EPM/API/Sets/$($set.setId)/Policies/Server/Search" -Method 'POST' -Headers $sessionHeader -Body $policyFilter

foreach ($policy in $policiesList.Policies) {

    # Parse the CreatedDate string into a DateTime object
    $createdDate = [datetime]::Parse($policy.CreatedDate)
    ...
    # Compare dates
    if ($createdDate -lt $oneMonthAgo) {
        Write-Log "$($policy.PolicyName) The date is more than one month old. The policy will be deleted..." INFO
        # Perform the action here
    } else {
        Write-Log "$($policy.PolicyName) The date is within the last month. The policy will be kept." INFO
    }
}
